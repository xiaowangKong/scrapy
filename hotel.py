...
def  get_productIdlist_by_type(baseurl, p, lasturl):  # p-skip
    ...
    try:
        soup = BeautifulSoup(page_code, 'lxml')  # id="plist"
        res = []
        hidcomment = soup.find('div', attrs={'class': 'shop-wrap'}).find('div', attrs={'class': 'content'}).find('ul', attrs={'class':'hotelshop-list'})
        #print(hidcomment)
...
def get_shuma_link_cat(root, category):  # <div class="category-item m" data-idx="3">
    ...
    try:
        soup = BeautifulSoup(page_code, 'lxml')  # id="plist"
        res = []
        hidcomment = soup.find('div', attrs={'class': 'section Fix'}).find('div', attrs={'class': 'nav'}).find_all('div',attrs={'class': 'type'}, recursive=False)
